...
Finally, we explored associations between human values and beliefs that people associated with the cryptocurrency topic. Moreover, we looked at the moderating role of the five beliefs on the association between value preferences and holding- vs. not-behavior (see Model 1 & 2) and separately for intention to hold vs. no intention to hold in the future vs. unsure (see Model 3 & 4).  

- Model 1 & 2: $Behavior_{yes/no} = \beta_{0} + \beta_{1:3}X_{Covariates} + \beta_{4}X_{OCH/STR} + \beta_{5}X_{CON/SEN} + \beta_{6:10}X_{belief-1:5} + \beta_{11:15}X_{OCH/STR}*X_{belief-1:5} + \beta_{16:20}X_{CON/SEN}*X_{belief-1:5} + e_i$

- Model 3 & 4: $Intention_{yes/no/unsure} = \beta_{0} + \beta_{1:3}X_{Covariates} + \beta_{4}X_{OCH/STR} + \beta_{5}X_{CON/SEN} + \beta_{6:10}X_{belief-1:5} + \beta_{11:15}X_{OCH/STR}*X_{belief-1:5} + \beta_{16:20}X_{CON/SEN}*X_{belief-1:5} + e_i$

Per outcome variable (behavior or intention), we estimated two models, one each for the openness to change vs. conservation and the self-transcendence vs. self-enhancement value dimensions. We chose to do this to avoid multicoliniarity and to gain an understanding as to which of the incompatible value typologies was a "better" predictor. A third reason for applying this strategy was to minimize the impact due to low sample sizes on the estimation algorithm and therefore on variance estimation error.

In each model-instance we first estimated the model with three typical covariates ($\beta_{1:3}$ = age, gender (women = 1, other = 0), and education (Abitur = 1, other = 0)) then included value preferences ($\beta_{4}$ = OCH/STR, $\beta_{5}$ = CON/SEN) and the five beliefs on cryptocurrencies ($\beta_{6:10}$ = investment opportunity, money exchange opportunity, state regulated, illegal commerce, and good time to buy). In a final step, we introduced the moderating effect of beliefs onto the effects of values on outcome variables ($\beta_{16:20}$).
...
Figure \@ref(fig:p12) depicts the distribution of each value preference for (a) people who have heard or haven't heard of digital- and/or cryptocurrencies as well as for (b) people who at the time of data collection have heard of digital- and/or cryptocurrencies and were holding, previously held, or never held such currencies.  
