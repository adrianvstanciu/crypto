...
[MB] Cryptocurrencies can be simplified as digital currencies, however, its possibilities go way beyond than that. In the late 90s some inventions that resemble digital money were developed but it was only in 2008 that Satoshi Nakamoto, who may be a person or a group of people, released what has become the most succesfull crpytocurrency so far: Bitcoin. In simple words, Nakamoto's entirely digital currency is based on a chain of mathematical algorithms (aka the blockchain technology). Their vision is that the centralized financial system as we know can one day become de-centralized, that is, financial transactions will be possible directly between the involved parties without the need to involve third ones (peer-to-peer transactions). By eliminating the role of banks and governmental bodies that secure trust in financial transactions, unnecessary taxes can be also removed and people can have direct control of their finances. Some discuss that there is even the possibility of abolishing global barriers in trade by removing currency exchange. Such ideas have since then entered the public knowledge domain especially because of its disruptive nature. It is undeniable that the topic is no longer in the underground of our society, but quite the contrary, it is here to stay. [MB]

[MB] Bitcoin's creation and release is believed to have a direct connection to the 2008 global financial crisis, which shook public confidence in the traditional financial system. The treatment given by the US government to help financial institutions recover from the crisis was very different from that offered to the public. While the former received financial aid, the latter suffered with an avalanche of houses and jobs losses [Reeves et al. 2014]. Hence, with such a turbulent economic environment as this one and the lack of trust by many in the traditional economic system, it was just a matter of time for alternatives to the traditional format of the financial system to begin to emerge. [MB] 

This research sets to explore reasons why some people are interested in such new form of finance whereas other are against it, or skeptical. Throughout the paper we refer to digital currencies and cryptocurrencies interchangebly, despite several differences between them [see @Cryptoverze2020]. We focus on human values as possible explanators and seek to understand associations with engagement with cryptocurrencies in the general German population. We look at the associations between human values in the Theory of Basic Human Values [TBHV, @Schwartz2012] and varying cryptocurrency items as formulated in a recent survey by the OECD [@OECD2019]. Data for the study stems from a quota sample in line with the demographics of the German general population and was collected in April 2022.

## Cryptocurrencies

[MB] As up to date as cryptocurrencies are, the most basic principle behind their conception is ancient: the cryptography. The purpose of using a technology over 4000 years old is to allow direct and private communication between two parties without the involvement of a third one (Gensler 2018). This way of communication if at the very heart of cryptocurrencies and the Internet (Binance Academy 2019). Cryptocurrencies combine cryptography with other technologies that made them feasible, like Bitcoin that uses the blockchain technology to safely store and verify its transactions. Thanks to the combination of an ancient (cryptography) and a rather new technology (blockchain) that cryptocurrencies are digital, private and decentralized currencies. [MB]

(Reeves, Aaron/ McKee, Martin/ Stuckler, David (2014). Economic suicides in the Great Recession in Europe and North America. In: British Journal of Psychiatry, 205(3), 246- 247. doi:10.1192/bjp.bp.114.144766.)
(Binance Academy (2019): History of Cryptography. Retrieved June 30, 2022 (https://academy.binance.com/en/articles/history-of-cryptography).
(Gensler, Gary (2018): 15.S12 Blockchain and Money. Fall 2018. Massachusetts Institute of Technology: MIT OpenCourseWare, https://ocw.mit.edu. Retrieved June 30, 2022(https://www.youtube.com/watch?v=EH6vE97qIP4).
...
Why should it matter if values inform people's interst in cryptocurrencies? What makes values ultimately relevant is that they are culture-informed, have a developmental trajectory that is lifelong [@Smallenbroeketal2022], while they can change due to specific life events such as entering parenthood [@Lonnqvistetal2018], but also due to drastic changes in society such as due to the Covid-19 Pandemic [@Danieletal2022]. Cryptocurrencies hold several similarities with other technological advancements such as the Internet and mobile phones. Compared to previous advancements however, the notion of cryptocurrencies is also in a way an economic and a political statement [Mariana = do you have a citation for this? I will try to find!]. Though this technology is still in its onset, there are reasons to expect that it will impact people's financial awareness and potentially people's dependencies on established monetary and governmental institutions [Mariana = do you have a citation for this? I will try to find!].
...
All the present analyses were carried out in R [@R] and using varying packages. These packages are referred to in the respective scripts (see GitHub repository [add here link]).
...
Finally, we explored associations between human values and beliefs that people associated with the cryptocurrency topic and how together they impacted behavior and intention. Four models were estimated and this was done separately for behavior and intention.

- **Model 1**: $Behavior/Intention_{yes/no} = \beta_{0} + \beta_{1:3}X_{Covariates}$
- **Model 2**: $Behavior/Intention_{yes/no} = \beta_{0} + \beta_{1:3}X_{Covariates} + \beta_{4}X_{OCH} + \beta_{5}X_{SEN}$
- **Model 3**: $Behavior/Intention_{yes/no} = \beta_{0} + \beta_{1:3}X_{Covariates} + \beta_{4}X_{OCH} + \beta_{5}X_{SEN} + \beta_{6:10}X_{belief-1:5}$
- **Model 4**: $Behavior/Intention_{yes/no} = \beta_{0} + \beta_{1:3}X_{Covariates} + \beta_{4}X_{OCH} + \beta_{5}X_{SEN} + \beta_{6:10}X_{belief-1:5} + \beta_{11:15}X_{OCH}*X_{belief-1:5} + \beta_{16:20}X_{SEN}*X_{belief-1:5}$

Model 1 included the intercept, and age, gender (1 = female, 0 = other), and education (1 = Abitur, 0 = other) as covariates. Abitur was selected because it is the highest possible school degree in the German education system. Model 2 added main effects of openness to change and self-enhancement. Model 3 added main effects of beliefs associated with cryptocurrencies. Finally, Model 4 included interaction terms between value preferences and beliefs.

We chose to include only openness to change and self-enhancement as predictors because (a) predictions regarding their effects were made and (b) this allowed us to avoid multicoliniarity that values with incompatible contents might bring to model estimation. Results based on regressions with non-hypothesized effects of incompatible value preferences are presented in the Appendix. 

There were values missing at random on each of the five beliefs associated with cryptocurrencies. The missing values were estimated with a multivariate imputation technique by chained equations. This was done in R using the package _mice_ [@Buurenetal2011]. The approach is iterative and imputes missing information on a variable-by-variable basis according to conditional densities. Between 10 and 20 iterations are considered sufficient to arrive at a robust imputation model. Here, 10 iterations were used. We produced 5 imputed complete datasets which were pooled for estimation of models 1 to 4. Robustness of these imputations is provided in the Appendix.

Model fit improvement in nested models was evaluated using a loglikelihood comparison test on the imputed data (see above). A nested model is generally considered as contributing to variance explanation if the fuller model (with more predictors) has a loglikelihood that is greater than the loglikelihood of the emptier model (with fewer predictors). That is, if the comparison test is associated with a significant _p_-value. 
...
The present data diverged in several ways from representative surveys and therefore from what it is expected to be the demographic structure in the German general population when human values were most recently recorded in the refined-TBHV (also see the Appendix). Notable discrepancies could be observed in view of age: the 30-40 years old group was the most over-represented in the present study with `r age3040`% over the value in the ESS-2018, which was followed by the 50-60 group with `r age5060`% and the 18-30 group with `r age1830`% respectively over the value in the ESS-2018. The age groups 60-70 (`r age6070`%) and 70+ (`r age70`%) were on the other hand underrepresented when compared to the same survey.
...
```{r reasons,fig.cap="Mentioned reasons for holding or not holding cryptocurrencies",fig.subcap=c("Holds now or held in the past","Doesn't hold and never held"),fig.asp=1,fig.ncol=2,out.width=".49\\linewidth",fig.pos="H"}

theme1 <- theme(axis.text.x = element_text(angle = 75, vjust = 0.9, hjust=1,size=14),
                 panel.background = element_rect(fill="white", color = "grey50"),
                 panel.grid.major.y = element_line(color="grey90"),
                 axis.title.y = element_text(vjust= 1, hjust=1, angle = 90,size=14)
)
